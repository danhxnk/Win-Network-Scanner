...
cls && for /l %%a in (1,1,254) do @Title Checking %NW%.%%a && @for /f "tokens=3 delims=: " %%a in ('ping %NW%.%%a -n 1 -w 60  ^| find /i "ttl"') Do SET IP=%%a && Call :Step

Goto End

:Step
curl http://%IP% --connect-timeout 0.5 --max-time 3 > NUL 2> NUL
IF %ERRORLEVEL% EQU 0 (ECHO %IP% http status: Port 80 Open)	ELSE (ECHO %IP% http status: Port 80 Closed)

curl https://%IP% --connect-timeout 0.5 --max-time 3 > NUL 2> NUL
...
for /f "tokens=*" %%a in ('curl --connect-timeout 0.5 max-time 3 -get https://api.maclookup.app/v2/macs/%MAC:~0,9%00-00-00/company/name -s') do ECHO %IP% Nic Vendor: %%a
ECHO.
